...
class CollectionObj {
    cards: Array<Card> = [];
    discoverCardsSeen = new Map<string, number>();
    ...
    incDiscoverSeenCount(card: Card) {
    ...
    private async discover(
        ...
        var orderedCards = cards
            ...
            .map((tup) => tup.card);
        for (var i = 0; i < n; i++) {
            const card = <Card>orderedCards[i];
            choices.push(card);
            this.incDiscoverSeenCount(card);
        }
        var output = new DeferredPromise<Card | null>();
        var prompt = new Prompt("Choose up to one card:");
        const cardCallback = async (card: Card) => {
