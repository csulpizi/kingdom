...
class MainDisplayObj {
    showDefaultHud() {
        ...
        this.writeLine(coloredString(`---`, "gray"));
        this.writeLine(`Cards in hand (${Collection.hand.length}): ${Collection.hand.map((c) => c.name).join(", ")}`);
        this.writeLine(`Lands in play (${Collection.inPlay.length}): ${Collection.inPlay.map((c) => c.name).join(", ")}`);
...
//# sourceMappingURL=mainDisplay.js.map