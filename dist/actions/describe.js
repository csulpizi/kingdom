...
export async function describeAction() {
    ...
    for (var card of Collection.hand) {
        ...
        if (playable)
            continue;
        MainDisplay.writeLine(coloredString(`(You cannot play ${card.name}: ${reason})`, "gray"));
    }
    ...
    MainDisplay.writeLine("");
    describePile("Cards in graveyard:", "No cards in graveyard", Collection.graveyard);
    MainDisplay.writeLine("");
...
function describePile(header, emptyHeader, cards) {
    if (cards.length > 0) {
